...
export class MarkupComponent implements OnInit {
  ...
  async loadMarkups() {
    try {
      ...
      const response = await this.markupService.getMarkupsBySeasonId(this.seasonID, token).toPromise();
  ...
  async onSubmitMarkupDetails(markup: any) {
    ...
    try {
      ...
      const response = await this.markupService.updateMarkup(markup.id, markup.form.value, token).toPromise();
  ...
  async onAddNewMarkup() {
    ...
    try {
      ...
      const response = await this.markupService.addMarkupToSeason(this.seasonID, this.newMarkupForm.value, token).toPromise();
  ...
  async onDeleteMarkup(markupId: number) {
    ...
    try {
      const response = await this.markupService.deleteMarkup(markupId, token).toPromise();
